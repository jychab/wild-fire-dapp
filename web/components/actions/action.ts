import { ActionAdapter } from '@/utils/actions/actions-config';
import {
  ActionSupportStrategy,
  BASELINE_ACTION_BLOCKCHAIN_IDS,
  BASELINE_ACTION_VERSION,
  defaultActionSupportStrategy,
} from '@/utils/actions/actions-supportability';
import { isUrlSameOrigin } from '@/utils/helper/blinks';
import { proxify } from '@/utils/helper/endpoints';
import { ExtendedActionGetResponse } from '@/utils/types/blinks';
...
import { MultiValueActionComponent } from './multivalue-action-coponent';
import { SingleValueActionComponent } from './single-action-value-component';
...
export class Action {
  ...
  private constructor(
    ...
    private readonly _post?: PostContent
  ) {
  ...
  static hydrate(
    ...
    return new Action(url, data, metadata, supportStrategy);
  }
