...
import { BlockchainIds, getShortBlockchainName } from './caip-2';
import { ACTIONS_SPEC_VERSION } from './dependency-version';
  ...
  if (!blockchainSupported) {
    const blockchainMessage =
      notSupportedActionBlockchainNames.length === 1
        ...
        : `Action blockchains ${notSupportedActionBlockchainNames.join(
            ', '
          )} are not supported by your Blink client.`;
